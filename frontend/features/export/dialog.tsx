...
import { saveCsv, savePng, saveSvg, svgReport } from "@/lib/export"
...
export function ExportDialog({ open, pattern, name, shape, onOpen }: Props) {
  ...
  const patch = (next: Partial<ExportOpts>) => setOpts((value) => ({ ...value, ...next }))
  const zoomPreview = (next: number) => setPreviewZoom(Math.min(150, Math.max(25, next)))

  const handlePreviewWheel = (event: React.WheelEvent<HTMLDivElement>) => {
    event.preventDefault()

    const viewport = event.currentTarget
    const nextZoom = Math.min(150, Math.max(25, previewZoom + (event.deltaY < 0 ? 10 : -10)))
  ...
  const preview = pattern ? svgReport(pattern, opts, name) : ""
  const previewSize = svgSize(preview)
    ...
    <Dialog open={open} onOpenChange={onOpen}>
      ...
      >
        <DialogHeader className="border-b px-6 py-4">
          ...
          <DialogDescription>先确认颜色编号、网格和材料清单，再导出最终文件。</DialogDescription>
        </DialogHeader>
            ...
            <div className="p-5">
              ...
              <FieldGroup>
                <Field>
                  ...
                  <ToggleGroup
                    value={[opts.mode]}
                    onValueChange={(items) => items[0] && patch({ mode: items[0] as ExportOpts["mode"] })}
                    variant="outline"
                    spacing={1}
                ...
                <Field>
                  ...
                  <Slider
                    ...
                    step={1}
                    value={[opts.cellSize]}
                    onValueChange={(next) => patch({ cellSize: Array.isArray(next) ? next[0] : next })}
                  />
                  <FieldDescription>大图会自动限制总尺寸，避免浏览器内存溢出。</FieldDescription>
                </Field>
                ...
                <Field>
                  ...
                  <ToggleGroup
                    value={[opts.shape]}
                    onValueChange={(items) => items[0] && patch({ shape: items[0] as BeadShape })}
                    variant="outline"
                    spacing={1}
                    className="grid w-full grid-cols-2"
                  >
                ...
                <Separator />
                <ToggleField title="显示细网格" description="每个豆格的边界线" checked={opts.grid} onChange={(grid) => patch({ grid })} />
                <ToggleField title="显示坐标" description="图案四周的行列编号" checked={opts.coords} onChange={(coords) => patch({ coords })} />
                <ToggleField title="拼板分界" description="每 29 格加粗一条分界线" checked={opts.boards} onChange={(boards) => patch({ boards })} />
                <ToggleField title="显示颜色编号" description="在非空豆格内写入色号" checked={opts.labels} onChange={(labels) => patch({ labels })} />
                <ToggleField title="材料清单" description="包含颜色、数量和占比" checked={opts.legend} onChange={(legend) => patch({ legend })} />

                {opts.mode === "pattern" && (
                  <ToggleField title="透明背景" description="空位保持透明" checked={opts.transparent} onChange={(transparent) => patch({ transparent })} />
                )}

                <Separator />
                <Field>
                  <FieldTitle>页脚署名</FieldTitle>
                  <FieldDescription>默认显示 Designed by Pixoras，可选择添加作者名称。</FieldDescription>
                </Field>
                <ToggleField
                  title="作者署名"
                ...
                <Field>
                  ...
                  <ToggleGroup
                    ...
                    onValueChange={(items) => items[0] && patch({ scale: Number(items[0]) as 2 | 4 })}
                    variant="outline"
                    spacing={1}
                    className="grid w-full grid-cols-2"
                  >
          ...
          <section className="flex min-h-[240px] min-w-0 flex-col bg-neutral-950 text-neutral-50 md:min-h-0">
            <div className="flex min-h-14 items-center justify-between border-b border-white/10 px-4">
              ...
              <div className="flex items-center gap-3">
                <span className="hidden text-xs text-neutral-400 lg:inline">滚轮缩放</span>
                <div className="flex items-center rounded-lg border border-white/15 bg-white/5 p-0.5 shadow-sm">
                  <Button
                    ...
                    aria-label="缩小预览"
                  >
                    <span aria-hidden className="text-lg leading-none font-semibold">−</span>
                  </Button>
                  <span className="min-w-14 text-center font-mono text-xs font-medium text-white">{previewZoom}%</span>
                  <Button
                    variant="ghost"
                    size="icon-sm"
                    className="size-8 bg-white/8 text-white hover:bg-white/15 hover:text-white disabled:bg-transparent disabled:text-neutral-600"
            ...
            <div
              className="min-h-0 flex-1 overscroll-contain overflow-auto p-5"
            ...
            >
              {pattern && (
                <div className="min-h-full min-w-full">
                  <div
                    className="mx-auto shrink-0 overflow-hidden shadow-xl [&_svg]:block [&_svg]:size-full [&_svg]:max-w-none"
                    style={{
                      width: `${previewZoom}%`,
        ...
        <DialogFooter className="border-t bg-background px-6 py-4">
          <Button variant="outline" disabled={!pattern || !!busy} onClick={() => run("csv")}>CSV 用量表</Button>
          <Button variant="outline" disabled={!pattern || !!busy} onClick={() => run("svg")}>SVG</Button>
